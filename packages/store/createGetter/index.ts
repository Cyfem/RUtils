...
export type ReducedData<G extends StoreGetter<any>, M extends GetterNameMap<G>> = {
  [K in keyof M as M[K]]: G[K extends keyof G ? K : never] extends (store: any) => infer R
    ? R
    : never;
};
/**
 ...
 */
export const createStoreGetterMemo = <S, G extends StoreGetter<S>, M extends GetterNameMap<G>>(
  store: ReturnType<typeof createStateStore<S>>,
  getters: G,
  getterNameMaps: M,
) => {
